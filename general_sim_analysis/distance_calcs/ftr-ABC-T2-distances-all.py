...
for file in glob.glob('./stripped/*nc'):
    ...
    for j in range(len(labels)):
        #d = md.compute_distances(t,[[res_ca[j],4156]]) #ShHTL7
        d = md.compute_distances(t,[[res_ca[j],4070]]) #AtD14 with GR24
        n_frames = t.n_frames
        ...
        np.save('./analysis/'+file.split('/')[-1]+'_ftr-ABC-T2-distance-%s.npy'%labels[j] ,dis)
